...
export async function GET(req: Request) {
  ...
  const [{ data: p }, plan, { data: credits }] = await Promise.all([
    db.from("profiles").select("full_name,email,canonical,onboarding_done,autofill_prefs").eq("id", u.id).single(),
    planOf(db, u.id),
  ...
  return NextResponse.json({
    name: p.full_name, email: p.email, plan, credits: plan === "free" ? credits?.balance ?? 3 : "unlimited", fields: applyPrefs(fillFields(d), prefs), prefs: { essays: prefs.essays, coverLetter: prefs.coverLetter, resume: prefs.resume, hoursPerWeek: prefs.hoursPerWeek }, skills: d.skills.map((s) => s.name),
