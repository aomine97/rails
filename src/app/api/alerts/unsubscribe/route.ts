...
export async function GET(req: Request) {
  ...
  const nudges = new URL(req.url).searchParams.get("what") === "nudges";
  const { data } = await supabaseAdmin().from("profiles").update(nudges ? { nudges_enabled: false } : { alerts_enabled: false }).eq("unsubscribe_token", t).select("id").maybeSingle();
  const body = data ? `You are unsubscribed from Rails ${nudges ? "follow-up reminders" : "job alerts"}. Turn them back on any time under Settings.` : "That link is no longer valid.";
