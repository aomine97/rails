...
import type { Adapter, AtsKind, CompanyRef, FetchLike, RawJob } from "./types";
...
export async function fetchCompanyJobs(c: CompanyRef, fetchImpl?: FetchLike): Promise<RawJob[]> {
  const a = adapters[c.ats];
  return a ? a.fetchJobs(c, fetchImpl) : [];
}
