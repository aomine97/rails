...
- Next item: Phase 2, Tagger throughput (nightly bulk run), then Adapters (iCIMS, Workable, Jobvite, Oracle, Taleo). Phase 1 is complete.
...
- STRIPE SETUP (Ilyas, ~15 min, then billing is live): 1) dashboard.stripe.com -> Products: create "Rails Pro" with two recurring monthly prices ($25 and $15) and "Semester Pass" with two one-time prices ($79 and $49). 2) Copy the four price IDs (price_...) into .env.local and Vercel as STRIPE_PRICE_PRO_MONTHLY, STRIPE_PRICE_PRO_MONTHLY_STUDENT, STRIPE_PRICE_SEMESTER, STRIPE_PRICE_SEMESTER_STUDENT; STRIPE_SECRET_KEY = the sk_test_/sk_live_ key. 3) Developers -> Webhooks -> add endpoint https://rails-psi.vercel.app/api/stripe/webhook with events checkout.session.completed, customer.subscription.created, customer.subscription.updated, customer.subscription.deleted, charge.refunded; copy the signing secret into STRIPE_WEBHOOK_SECRET. 4) Settings -> Billing -> Customer portal: enable, allow cancel + update payment method. 5) Run supabase/migrations/0005_billing.sql in the SQL editor. 6) Redeploy Vercel. Test with card 4242 4242 4242 4242 in test mode.
