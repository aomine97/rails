...
- Next item: Phase 2, Company discovery (careers-page crawl for ATS slugs, the 123 "unknown" rows, auto-add company from pasted URLs), then Workday page-3 bug.
...
- [x] Adapters: Workable (v3 list + v2 detail), Jobvite (HTML list + detail), iCIMS (HTML search pages + JSON-LD job pages), Oracle Recruiting Cloud (hcmRestApi, keyed by careers_url, site CX_1 default) written against documented/observed shapes with fixture tests; poller now polls every ATS that has an adapter. UNVERIFIED LIVE: run `npx tsx scripts/check-feeds.mjs 3 workable,jobvite,icims,oracle` on your machine and report; iCIMS and Jobvite are HTML scrapes so a markup difference means 0 jobs, not an error. Taleo skipped (3 companies, undocumented POST API). +82 companies
